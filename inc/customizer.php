...
function simone_customize_preview_js() {
	wp_enqueue_script( 'simone_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview', 'jquery' ), '20130508', true );
}
...
function simone_sanitize_archive( $value ) {
	if ( ! in_array( $value, array( 'excerpt', 'content' ), true ) ) {
		$value = 'excerpt';
	}
...
function simone_customizer_css() {
	$header_color = get_theme_mod( 'simone_header_color', '#0587BF' );
	$link_color   = get_theme_mod( 'simone_link_color', '#000000' );
	?>
	<style type="text/css">
		.site-branding {
		...
		.border-custom {
			border: <?php echo sanitize_hex_color( $link_color ); ?> solid 1px;
		}
